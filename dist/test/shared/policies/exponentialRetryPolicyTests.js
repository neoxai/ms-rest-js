...
describe("exponentialRetryPolicy", function () {
    describe("shouldRetry()", function () {
        ...
        var policy = new exponentialRetryPolicy_1.ExponentialRetryPolicy({}, nextPolicy, new requestPolicyOptions_1.RequestPolicyOptions());
        var request = new httpRequest_1.HttpRequest({ method: "GET", url: "https://www.example.com" });
        it("should return when no response is given", function () {
        ...
        it("should not retry when response status code is 300", function () {
            assert.strictEqual(policy.shouldRetry({ response: new inMemoryHttpResponse_1.InMemoryHttpResponse(request, 300, {}) }), false);
        });
        it("should retry when response status code is 408", function () {
            assert.strictEqual(policy.shouldRetry({ response: new inMemoryHttpResponse_1.InMemoryHttpResponse(request, 408, {}) }), true);
        });
        it("should not retry when response status code is 500", function () {
            assert.strictEqual(policy.shouldRetry({ response: new inMemoryHttpResponse_1.InMemoryHttpResponse(request, 500, {}) }), true);
        });
        it("should retry when response status code is 501", function () {
            assert.strictEqual(policy.shouldRetry({ response: new inMemoryHttpResponse_1.InMemoryHttpResponse(request, 501, {}) }), false);
        });
        it("should retry when response status code is 505", function () {
            assert.strictEqual(policy.shouldRetry({ response: new inMemoryHttpResponse_1.InMemoryHttpResponse(request, 505, {}) }), false);
        });
    });
